...
@property (copy, nonatomic, setter=pb_setInfiniteTopScrollHandler:, getter=pb_infiniteTopScrollHandler)
void(^pb_infiniteTopScrollHandler)(UIScrollView* scrollView);

// Infinite scroll state
@property (nonatomic, setter=pb_setInfiniteTopScrollState:, getter=pb_infiniteTopScrollState)
PBInfiniteTopScrollState pb_infiniteTopScrollState;

// A flag that indicates whether scroll is initialized
@property (nonatomic, setter=pb_setInfiniteTopScrollInitialized:, getter=pb_infiniteTopScrollInitialized)
BOOL pb_infiniteTopScrollInitialized;

// Extra padding to push indicator view below view bounds.
// Used in case when content size is smaller than view bounds
@property (nonatomic, setter=pb_setInfiniteTopScrollExtraBottomInset:, getter=pb_infiniteTopScrollExtraBottomInset)
CGFloat pb_infiniteTopScrollExtraBottomInset;
...
- (void)addInfiniteTopScrollWithHandler:(void(^)(UIScrollView* scrollView))handler {
    ...
    self.pb_infiniteTopScrollInitialized = YES;
}
...
- (void)removeInfiniteTopScroll {
    ...
    self.pb_infiniteTopScrollInitialized = NO;
}
...
- (void)finishInfiniteTopScrollWithCompletion:(void(^)(UIScrollView* scrollView))handler {
    if(self.pb_infiniteTopScrollState == PBInfiniteTopScrollStateLoading) {
        [self pb_stopAnimatingInfiniteTopScrollWithCompletion:handler];
    }
}
...
- (void)pb_setInfiniteTopScrollState:(PBInfiniteTopScrollState)state {
    objc_setAssociatedObject(self, kPBInfiniteTopScrollStateKey, @(state), OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    TRACE(@"pb_setInfiniteTopScrollState = %ld", (long)state);
}

- (void)pb_setInfiniteTopScrollHandler:(void(^)(UIScrollView* scrollView))handler {
    objc_setAssociatedObject(self, kPBInfiniteTopScrollHandlerKey, handler, OBJC_ASSOCIATION_COPY_NONATOMIC);
}

- (void(^)(UIScrollView* scrollView))pb_infiniteTopScrollHandler {
    return objc_getAssociatedObject(self, kPBInfiniteTopScrollHandlerKey);
}

- (void)pb_setInfiniteTopScrollExtraBottomInset:(CGFloat)height {
...
- (void)pb_setInfiniteTopScrollInitialized:(BOOL)flag {
...
- (void)pb_handlePanGesture:(UITapGestureRecognizer*)gestureRecognizer {
    if (gestureRecognizer.state == UIGestureRecognizerStateEnded) {
        [self pb_scrollToInfiniteTopIndicatorIfNeeded];
    }
}
...
- (void)pb_callInfiniteTopScrollHandler {
    if(self.pb_infiniteTopScrollHandler) {
        self.pb_infiniteTopScrollHandler(self);
    }
    TRACE(@"Call handler.");
}
...
- (CGFloat)pb_infiniteTopIndicatorRowHeight {
...
- (void)pb_positionInfiniteTopScrollIndicatorWithContentSize:(CGSize)size {
...
- (void)pb_startAnimatingInfiniteTopScroll {
	
    UIView* activityIndicator = [self pb_getOrCreateActivityIndicatorView];
    
    [self pb_positionInfiniteTopScrollIndicatorWithContentSize:self.contentSize];
    ...
    contentInset.top += [self pb_infiniteTopIndicatorRowHeight];
    ...
    // Save extra inset
    self.pb_infiniteTopScrollExtraBottomInset = extraBottomInset;
    
    TRACE(@"extraBottomInset = %.2f", extraBottomInset);
    
    self.pb_infiniteTopScrollState = PBInfiniteTopScrollStateLoading;
...
- (void)pb_stopAnimatingInfiniteTopScrollWithCompletion:(void(^)(UIScrollView* scrollView))handler {
    UIView* activityIndicator = self.infiniteTopScrollIndicatorView;
    UIEdgeInsets contentInset = self.contentInset;
    ...
    contentInset.top -= self.pb_infiniteTopScrollExtraBottomInset;
    ...
    [self pb_setScrollViewContentInset:contentInset animated:YES completion:^(BOOL finished) {
        ...
        self.pb_infiniteTopScrollState = PBInfiniteTopScrollStateNone;
...
//
- (void)pb_scrollToInfiniteTopIndicatorIfNeeded {
	
    if (![self isDragging] && self.pb_infiniteTopScrollState == PBInfiniteTopScrollStateLoading) {
		
        CGFloat indicatorRowHeight = [self pb_infiniteTopIndicatorRowHeight];
...
- (void)pb_setScrollViewContentInset:(UIEdgeInsets)contentInset
...
- (void)pb_setContentSize:(CGSize)contentSize {
	...
	if(self.pb_infiniteTopScrollInitialized) {
		[self pb_positionInfiniteTopScrollIndicatorWithContentSize:contentSize];
	}
}
...
- (void)pb_scrollViewDidScroll:(CGPoint)contentOffset {
	...
	CGFloat actionOffset = -([self pb_infiniteTopIndicatorRowHeight]);
	...
	if ([self isDragging] && hasActualContent && contentOffset.y < actionOffset) {
		if (self.pb_infiniteTopScrollState == PBInfiniteTopScrollStateNone) {
			TRACE(@"Action.");
			
			[self pb_startAnimatingInfiniteTopScroll];
			
			// This will delay handler execution until scroll deceleration
			[self performSelector:@selector(pb_callInfiniteTopScrollHandler)
					   withObject:self
					   afterDelay:0.1
...
- (void)setInfiniteIndicatorView:(UIView*)indicatorView {
	[self setInfiniteTopScrollIndicatorView:indicatorView];
}

- (UIView*)infiniteIndicatorView {
	return [self infiniteTopScrollIndicatorView];
}
